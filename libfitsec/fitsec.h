...
extern "C" {
	...
	enum FitSecEngineFlags {
		...
		FS_SEND_AA_CERT_IF_BOTH_REQUESTED,

		/** request both AT and AA certificates if AA certificate needs to be requested */
		FS_REQUEST_AT_CERT_WITH_AA,
	};

	typedef struct FitSecConfig
	{
		const char                * storage;        // deprecated. path to load and store certificates
		int                         hexadecimal;    // certificate data is in hexadecimal form
		unsigned int                version;        // default protocol version
		int                         certPeriod;     // set to -1 to do not send certificates automatically,
		                                            // set to  0 to send certificates in each CAM
		unsigned int                flags;          // see FitSecEngineFlags
		const FSCryptEngineConfig * crypt;          // crypto engine to be used

		FitSec_Event_Fn           * cbOnEvent;      // user function to be called when some event has been occured (see FSEventId)
		void                      * cbUser;         // user pointer to be passed to the function
	}FitSecConfig;
	...
	/** Load certificates from the path cpecified in storage. Deprecated. */
	...
	typedef struct FSMessageInfo {
		...
		const char *          errmsg;         // message to describe an error
		char *                payload;        
		int                   payloadSize;
	 ...
	 *  generationTime | in:                                        | -
	 *  expirationTime | - unsupported                              | - unsupported
